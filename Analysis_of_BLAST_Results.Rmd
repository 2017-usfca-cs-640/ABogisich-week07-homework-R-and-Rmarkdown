...
author: "Allison Bogisich asbogisich@dons.usfca.edu"
...
```
Fig. 2 Frequency of Mismatched Microbial Sequences from Female Samples
...
joined_blast_data_metadata %>%
  ...
  hist(main = "Male Microbial Survey",
       xlab = "Number of Mismatches")
```
Fig. 3 Frequency of Mismatched Microbial Sequences from Male Samples
...
joined_blast_data_metadata %>%
  filter(sex_s == "female") %>%
  pull(bitscore) %>%
  hist(main = "Bitscores for Female Sequences",
       xlab = "Bitscore Value")
```
Fig. 4 Bitscore Distribution for Sequenced Female Microbiome Samples

The bit score (S) formula takes into account gaps in the sequence; the higher the score, the better the alignment for the sequence. High bit scores can occur when the alignment length is longer, even when there are more mismatches than for other hits. Here in figure 4 we see that the distribution is strongly skewed left/ negatively skewed for sequences derived from female skin samples. With the tail trailing left, the majority of the sequenced microbial DNA had highly significant bitscores. The mode score falls between 400-425.
...
joined_blast_data_metadata %>%
  filter(sex_s == "male") %>%
  pull(bitscore) %>%
  hist(main = "Bitscores for Male Sequences",
       xlab = "Bitscore Value")
```
Fig. 5 Bitscore Distribution for Sequenced Male Microbiome Samples

For microbial sequences derived from male skin samples, no strong pattern of distribution emerges.The most common bitscore falls between 150-200, with a smaller, secondary clustering of scores falling between 325-400. The largest frequency for males is higher (~1800), and occurs at a lower bitscore (150-200) versus the female bitscore mode of 400-425, with a lower frequency (~1000).
